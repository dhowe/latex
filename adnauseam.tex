...
Requests marked as blockable are disallowed at the network level, mimicing the behavior of other existing blockers (uBlock, AdBlock Plus, Adblock, Adguard, etc.) which perform blocking on some percentage of requests and hiding on some percentage of the remainder. The only difference for AdNauseam is that a subset of requests which may possibly be blocked in other blockers must be allowed in AdNauseam; specifically those that result in, directly or indirectly, visual advertisements. \footnote{Interestingly it is this standard combination of functions---hiding and blocking---that Google claims violates its Terms of Service, a claim discussed below in the \emph{Distribution} section.} At the element hiding level, the detection module is invoked incrementally, via content-scripts, as page elements are loaded (or dynamically generated) and inserted into the DOM. Elements marked for hiding are assigned a CSS class that sets their display to invisible, and the surrounding DOM is collapsed so as not to leave blank space on the page. Each hidden element, generally a visual ad, is then passed to the \emph{Extraction} module for further processing.
...
\begin{figure}[!t]
\centering
\includegraphics[width=3in]{images/vault.png}
\caption{AdNauseam's AdVault visualization.}
\label{fig:vault}
\vspace{10mm}
\includegraphics[width=3in]{images/inspector.png}
\caption{Inspecting a single ad in the AdVault.}
\label{fig:inspector}
\vspace{10mm}
\includegraphics[width=3in]{images/menu.png}
...
\begin{figure}[!t]
\centering
\includegraphics[width=3in]{images/thirdparty.png}
\caption{Number of distinct 3rd-parties contacted.}
\label{fig:thirdparty}
\vspace{10mm}
\includegraphics[width=3in]{images/memory.png}
\caption{Overall memory footprint (MB).}
\label{fig:memory}
\vspace{10mm}
\includegraphics[width=3in]{images/loadtime.png}
...
\begin{figure}[!t]
\centering
\includegraphics[width=3in]{images/firstrun.png}
...
\bibitem{Gurses-0} G\"urses, Seda, Carmela Troncoso, and Claudia Diaz. “Engineering privacy by design.” Computers, Privacy \& Data Protection 14.3 (2011).

\bibitem{Gurses-1} G\"urses, Seda, Carmela Troncoso, and Claudia Diaz. “Engineering privacy by design reloaded.” Amsterdam Privacy Conference. 2015.
