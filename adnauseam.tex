...
[NOTE: If we are going to do this section, I think we need to specifically address the question in the above quotation, saying that for at least a subset of our users, who are already using a quality adblocker (like uBlock Origin), AdNauseam represents a hit to their personal privacy, hoever it makes up for it when consider the larger social good...]
...
To establish a conservative lower-bound on exposure, we set a constraint on design and implementation decisions that user exposure with AdNauseam must be strictly lower on all dimensions than with our baseline case of browsing with an unmodified browser. Conversely, we hypothesized that the current performance of uBlock, the open-source blocker with the best performance metrics, would provide an upper bound on risk exposure for the individual user. As AdNauseam must interact, at least minimally, with advertising servers in order to fulfill its functional requirements, it would necessarily expose users to more risk than the current state-of-the art blocker (e.g., uBlock), which does not have such a constraint. Notice that we refer specifically here to `'risk for \emph{the individual}', a distinction we return to below. In all case we were able to
...
% The importance of user click data for content ranking and placement has been extensively studied, see for example \cite{Grotov} for a recent review. A number of evaluation techniques for obfuscation strategies have been proposed, with search being the most frequently discussed \cite{Gervais,Balsa}. However, for AdNauseam, we must consider a number of distinct cases based on whether the user had installed an adblocker prior to using AdNauseam, and whether they have enabled the `'hiding' function of AdNauseam. Considering these two variables yields the two-by-two evaluation matrix based on the answer seen in Figure \ref{fig:matrix}. As AdNauseam does not collect or store data about users, we have no access to actual numbers here, and so must consider each case, referred to as A, B, C & D from the figure.
%
% Case A is the simplest to analyze. We assume that user in this category have an existing user profile and have previously been served some number, k, of advertisements. From these, they have clicked on some number c, now stored in their profile where 0 >= c <= k.  After installation of AdNauseam, there are no further true clicks (as no Ads are seen in case A), only AdNauseam clicks and potentially clicks on ads on DNT-respecting sites. In such a case, similar to the installation of
%
% Additionally these users are exposed to the full range of trackers and malware
%
